...
import secrets
from .helpers import token_required, permission_required
...
class Role(db.Model):
    ...
    add and remove permissiosn from roles'''
    __tablename__ ='roles'
    id=db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(150), nullable=False, default ='PUBLIC')
    permissions = db.Column(db.Integer)
    user = db.relationship('User', rackref='role', lazy='dynamic')
...
class Staff(db.Model):
    ...
    role = db.relationship('Role', backref='staff', lazy='dynamic')
...
class User(db.Model, UserMixin):
    ...
    def assign_classrooms(self):
        if self.has_permission(Permission.CLASS):
            ...
            for assignment in teacher_assignments:
                self.follow_classrooms(assignment.id)  # Assuming this method correctly associates the classroom
    ...
    def is_following(self, classroom_view):
        if classroom_view.id is None:
            return False
        return self.classroom_views.filer_by(classroom_view = classroom_view.id).first() is not None
